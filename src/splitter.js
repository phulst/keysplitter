...
class Splitter {
    ...
    restorePrivateKey(splitKey1, splitKey2) {
        ...
        return asHex(keySegments[0]) + asHex(keySegments[1]) + asHex(keySegments[2]);
    }
    ...
    createOTPKeys(length) {
        let pads = [];
        for (let i = 0; i < 3; i++) {
            pads[i] = crypto.randomBytes(length);
    ...
    splitInto3Segments(key) {
        ...
        let start = 0, end = 0;
        let bufpart = [];
        let partLen = Math.round(buf.length / 3);
        for (let i = 0; i < 3; i++) {
            if (i === 2) {
...
/**
 * converts a buffer or array of bytes to a hex string
 * @param buf
 * @returns {string}
 */
function asHex(buf) {
